 ...
 */
	class geradordeform
	{
		...
		function geradordeform($host,$user,$password,$db=false,$table=false,$project="Projeto criado pelo gerador de classes",$author="Leonardo Weslei Diniz <leonardoweslei@gmail.com>",$pasta_to_save="doc/",$dict=array()){
			$this->user 		= $user;
	...
	/*/*require_once(\"conf.php\");*/*/".implode("",$dep);
				$tmp=array();
				$tmp[]="<?php
	\$title=\"New ".$this->traducao($tablei).":\";
	include(\"head.php\");
$dep
$post
$mult
...
?>";
	...
	conf_require(\"/class/ATTRIBUTE_T_HERE.php\");");
				...
				$tmp=array();
				$tmp[]='<?
	$title="Listagem de '.$this->traducao($tablei).'s cadastradas";
	include("head.php");
	'.$dep.'
	'.$depp.'
	$'.$tablei.'s=new '.$tablei.'();
...
?>
				<form name="form_listing_'.$tablei.'" action="?del_'.$tablei.'.php" method="post" enctype="multipart/form-data">
					<table width="100%" border="0" cellpadding="5" cellspacing="1" class=\"form_edit_reg\">
...
<?
				...
				foreach($fields as $key=>$datafield){
					...
					$tmp[]='
							<td width="'.(100/($total_campos)).'%">
								...
								'.(($key==$id)?'<a href="javascript:void(0);" onclick="window.open(\\\'form_edit_'.$tablei.'.php?'.$id.'=\'.$'.$tablei.'->'.$id.'.\'\\\',\\\''.$tablei.'\\\');" alt="editar">':'').'
								\'.$'.$tablei.'->'.$key.'.\''.(($key==$id)?'</a>':'').'
								</font>
							</td>';
				}
	...
	if ($_COUNTER_RESULTS_==0){
		...
		$__PAGING__= "Exibindo de resultado de ".($_PAGE_INIT_+1)." a ".($_PAGE_NOW_==1?$_COUNTER_RESULTS_:(($_COUNTER_RESULTS_<($_MAX_RESULT_TO_PAGE_*$_PAGE_NOW_))?$_COUNTER_RESULTS_:$_PAGE_INIT_))."&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;".$_TOTAL_RESULTS_." Registros".(($_TOTAL_PAGES_>1)?"&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;$_TOTAL_PAGES_ Paginas&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;":"");
    if($_TOTAL_PAGES_ > 1 ){
        // Mostragem de pagina  
						...
						<tr class=\"listing_title\"> 
							<td height="9" colspan="'.($total_campos+1).'" align="center">
								<strong>
									...
									<input type="button" value="Novo" name="novo" onclick="window.open(\'?form_new_'.$tablei.'.php\',\'new_'.$tablei.'\');">
									</font>
								</strong>
							</td>
						</tr>
