	...
	function validafm()
	{
		...
		return true;
                                ...
                                foreach($fields as $key=>$datafield){
                                        ...
                                        }else if ($datafield['Key']=="MUL" && $this->types[$key]!='hidden' && $this->types[$key]!='dateh' && $this->types[$key]!='datetimeh')
                                        {
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                                        <select name=\"$key\"><option value=\"\">Selecione</option><?php echo \$tmp_$key;?></select>
                                                </td>
                                            </tr>";
                                ...
                                $tmp[]="
                                            <tr>
                                                <td colspan=\"2\">
                                                                        <input type=\"submit\" value=\"Enviar\"/>&nbsp;<input type=\"button\" value=\"voltar\" onclick=\"window.location.href='listing_".$tablei.".php';\"/>
                                                </td>
                                            </tr>
                                                </table>
                                        </form>
...
?>";
        ...
        ");
                                ...
                                foreach($fields as $key=>$datafield){
                                        $tmp2.='
                                                        <option value="'.$key.'"<?php echo (isset($_GET[\''.$key.'\'])?" selected":"");?>>'.$this->traducao($key).'</option>';
                                        $tmp3.='
        if(isset($_GET[\''.$key.'\']))
        {
        	$add_paginacao["'.$key.'"]=$_GET[\''.$key.'\'];
        	$where.=(strlen($where)>0?"AND ":"").\''.$key.' like "%\'.$_GET[\''.$key.'\'].\'%"\';
        	$queryb.=$_GET[\''.$key.'\'];
        }';
                                }
                                $tmp[]='<?php
        ...
        $pagination=new pagination(\''.$tablei.'\',$add_paginacao,$where."order by ".$order.$order2,"listing_'.$tablei.'.php","pg",10,9,false,array("<<","<",">",">>","<u style=\"font-size:16px;\">","</u>"),array(\'style="text-decoration:none;"\'));
        
        $dados=$'.$tablei.'s->search'.$tablei.'('.$param.'$order.$order2,(($pagination->page_now-1)*$pagination->max_results).",".($pagination->max_results));
?>
                                <input type="text" name="s" id="s" value="<?php echo $queryb;?>">
                                <select name="b" id="b">
                                '.$tmp2.'
                                </select>
                                <input type="button" onclick="window.location.href=\'?\'+brows(\'b\').value+\'=\'+brows(\'s\').value;" value="Buscar">
                                ...
                                foreach($fields as $key=>$datafield){
                                        ...
                                        $tmp[]='
                                                        <td width="'.(100/($total_campos)).'%">
                                                                <strong>
                                                                        <font  size="2" face="Verdana, Arial, Helvetica, sans-serif"><?php echo "<a style=\"text-decoration:none;\" href=\"?order='.$key.'&order2=".(($order=="'.$key.'" &&$order2==" ASC")?" DESC":" ASC")."\">";?>'.$this->traducao($key).'<?php echo "</a>";?></font>
                                                                </strong>
                                                        </td>';
                                }
                                $tmp[]='	
                ...
                echo \'
                                ...
                                foreach($fields as $key=>$datafield){
                                        ...
                                        <td width="'.(100/($total_campos)).'%">
                                                ...
                                                '.(($key==$id)?'<a href="javascript:void(0);" onclick="window.location.href=\\\'form_edit_'.$tablei.'.php?'.$id.'=\'.$'.$tablei.'->'.$id.'.\'\\\';" alt="editar">':'').'
...
?>
                                                <tr class="listing_title"> 
                                                        <td height="9" colspan="'.($total_campos).'">
                                                                <strong>
                                                                        <font  size="2" face="Verdana, Arial, Helvetica, sans-serif"><?php echo $pagination->pagination_advanced();?></font>
                                                                </strong>
                                                        </td>
                                                </tr>
                                                <tr class="listing_title"> 
