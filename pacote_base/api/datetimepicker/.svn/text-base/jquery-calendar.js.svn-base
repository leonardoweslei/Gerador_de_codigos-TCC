...
var popUpCal = {
	...
	dayNames: ['Dom','Seg','Ter','Qua','Qui','Sex','Sab'], // Names of days starting at Sunday
	monthNames: ['Janeiro','Fevereiro','Mar&ccedil;o','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'], // Names of months
	...
	formatDate: function(day, month, year, hour, minute) {
		...
		var LWDseconds=new Date();
		LWDseconds=LWDseconds.getSeconds();
		//append time info
		if (this.timeSeparators && this.timeSeparators.length > 1 && this.timeSeparators[0] && hour != null && minute != null) {
			dateString += this.timeSeparators[0] + this.hourString(hour) + this.timeSeparators[1] + ((minute >= 10) ? minute : ('0' + minute))+ this.timeSeparators[1] + LWDseconds;
	...
	showCalendar: function() {
		...
		var html = (this.closeAtTop ? '<div id="calendar_control">' +
			...
			'<span id="calendar_clear" style="width:55px;">&nbsp;&nbsp;&nbsp;</span><a id="calendar_current">' + this.currentText + '</a>' +
		...
		html += '</div>' + timeSelect + '<div id="calendar_header" style="height:2px;"></div><table id="calendar" cellpadding="0" cellspacing="0"><thead>' +
