...
/*$host = _HOST_MYSQL_SITE_;//host/endereço do banco
$user = _USER_MYSQL_SITE_;//usuário acesso
$password = _PASSWD_MYSQL_SITE_;//password do usuário
$dir_temp = _ROOT_DIR_SITE_."upload/";//diretório temporário
$dir_backup = _ROOT_DIR_SITE_."upload/";//diretório que irá guardar os backups
$database = _DB_MYSQL_SITE_;//especificando uma base para backup
  ...
  /*  $sql = $mysql_dump->dumpDB($database); //Takes all database backups
    ...
    */
    ...
    /*@param : $sql sql commands comes from MYSQL_DUMP::dumpDB() function
               [$sqlfilename]    Sql file name
    */
/*    if(isset($_GET['d']))$mysql_dump->download_sql($sql,"dump_".date("dmYHis")."_.sql");
    if(isset($_GET['s']))$mysql_dump->save_sql($sql,$dir_backup."dump_".date("dmYHis")."_.sql");
    // Print SQL
    if(isset($_GET['p']))echo nl2br($sql);
